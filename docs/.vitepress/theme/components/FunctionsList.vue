...
const result = computed(() => {
  if (search.value) {
    ...
    if (sortMethod.value === 'updated')
      ...
      fns.sort((a, b) => metadata.categories.indexOf(a.category || '') - metadata.categories.indexOf(b.category || ''))
    return fns
  }
})
...
<template>
  <div id="functions-list" class="grid grid-cols-[80px_auto] gap-y-2 mt-10">
    ...
    <div flex="~ wrap" gap="2" m="b-2">
      <button
        v-for="cate of metadata.categories"
        :key="cate"
        class="select-button"
...
<style>
.content:has(#functions-list) {
  max-width: 80vw !important;
}

.content-container:has(#functions-list) {
  max-width: 80vw !important;
}

.aside:has(+ .content #functions-list) {
